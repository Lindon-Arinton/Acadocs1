...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($type === 'canteen' && hasRole('admin','canteen')) {
        ...
        $exp  = (float)$_POST['expenses'];
        $db->prepare("INSERT INTO canteen_records (date,description,revenue,expenses,transaction_count) VALUES (?,?,?,?,?)")
           ->execute([$_POST['date'],$_POST['description'],$rev,$exp,(int)$_POST['transaction_count']]);
        $_SESSION['flash'] = ['type'=>'success','msg'=>'Canteen record added successfully!'];
    } elseif ($type === 'fund' && hasRole('admin','disbursing')) {
        $last    = $db->query("SELECT balance FROM school_funds ORDER BY date DESC LIMIT 1")->fetchColumn();
        $amount  = -abs((float)$_POST['amount']); // always a disbursement (negative)
        $balance = ($last ?: 0) + $amount;
        $db->prepare("INSERT INTO school_funds (date,category,description,particulars,amount,balance,prepared_by) VALUES (?,?,?,?,?,?,?)")
           ->execute([$_POST['date'],$_POST['category'],$_POST['description'],$_POST['particulars']??'',$amount,$balance,currentUser()['name']]);
        $_SESSION['flash'] = ['type'=>'success','msg'=>'Fund disbursement record added successfully!'];
    }
    header('Location: ' . BASE_URL . 'financial-reports'); exit;
}

$canteenRecords = $db->query("SELECT * FROM canteen_records ORDER BY date ASC")->fetchAll();
$fundRecords    = $db->query("SELECT * FROM school_funds ORDER BY date ASC")->fetchAll();
