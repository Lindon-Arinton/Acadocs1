...
requireLogin();
define('PAGE_TITLE', 'Time Records');
$db = getDB();

if ($_SERVER['REQUEST_METHOD'] === 'POST' && hasRole('admin','adas')) {
    $action = $_POST['action'] ?? '';
    if ($action === 'add') {
        $db->prepare("INSERT INTO time_records (date,employee_name,employee_id,time_in,time_out,status,remarks) VALUES (?,?,?,?,?,?,?)")
           ->execute([$_POST['date'],$_POST['employee_name'],$_POST['employee_id'],
                      $_POST['time_in']??null,$_POST['time_out']??null,$_POST['status'],$_POST['remarks']??'']);
        $_SESSION['flash'] = ['type'=>'success','msg'=>'Time record added.'];
    } elseif ($action === 'update') {
        $db->prepare("UPDATE time_records SET time_in=?,time_out=?,status=?,remarks=? WHERE id=?")
           ->execute([$_POST['time_in']??null,$_POST['time_out']??null,$_POST['status'],$_POST['remarks']??'',(int)$_POST['id']]);
        $_SESSION['flash'] = ['type'=>'success','msg'=>'Record updated.'];
    }
    header('Location: ' . BASE_URL . 'time-records?date=' . ($_POST['date'] ?? date('Y-m-d'))); exit;
}
...
foreach ($records as $r) { if (isset($summary[$r['status']])) $summary[$r['status']]++; }
