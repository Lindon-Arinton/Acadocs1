...
requireLogin();
define('PAGE_TITLE', 'Parent Meetings');
$db = getDB();

if ($_SERVER['REQUEST_METHOD'] === 'POST' && hasRole('admin','secretary')) {
    if ($_POST['action'] === 'add') {
        $actual   = (int)$_POST['actual_attendance'];
        $expected = (int)$_POST['expected_parents'];
        $rate     = $expected > 0 ? round($actual / $expected * 100, 2) : 0;
        $db->prepare("INSERT INTO parent_meetings (title,date,expected_parents,actual_attendance,attendance_rate) VALUES (?,?,?,?,?)")
           ->execute([$_POST['title'],$_POST['date'],$expected,$actual,$rate]);
    }
    header('Location: ' . BASE_URL . 'parent-meetings?success=1'); exit;
}

$meetings = $db->query("SELECT * FROM parent_meetings ORDER BY date DESC")->fetchAll();
