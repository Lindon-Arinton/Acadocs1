...
requireLogin();
define('PAGE_TITLE', 'Performance Analytics');
$db = getDB();

$year = $_GET['year'] ?? '2025-2026';

$byLevel   = $db->prepare("SELECT * FROM performance_by_level WHERE school_year=? ORDER BY grade_level");
$byLevel->execute([$year]); $byLevel = $byLevel->fetchAll();

$bySubject = $db->prepare("SELECT * FROM performance_by_subject WHERE school_year=? ORDER BY mps DESC");
$bySubject->execute([$year]); $bySubject = $bySubject->fetchAll();

$kpi = $db->prepare("SELECT * FROM kpi_snapshots WHERE school_year=? LIMIT 1");
$kpi->execute([$year]); $kpi = $kpi->fetch();
