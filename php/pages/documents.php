...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && hasRole('admin')) {
    ...
    if ($docId && $comment) {
        $db->prepare("INSERT INTO document_feedback (document_id,author,comment,date) VALUES (?,?,?,?)")
           ->execute([$docId, currentUser()['name'], $comment, date('Y-m-d')]);
        $db->prepare("UPDATE documents SET status='Reviewed' WHERE id=?")->execute([$docId]);
        header('Location: ' . BASE_URL . 'documents?success=1'); exit;
    }
}

$statusFilter = $_GET['status'] ?? 'all';
$where  = $statusFilter !== 'all' ? "WHERE d.status=?" : "WHERE 1";
$params = $statusFilter !== 'all' ? [$statusFilter] : [];

$stmt = $db->prepare(
    "SELECT d.*, t.name AS teacher_name FROM documents d
     JOIN teachers t ON t.id=d.teacher_id
     $where ORDER BY d.date_submitted DESC"
);
$stmt->execute($params);
$docs = $stmt->fetchAll();
  ...
  <div class="card-body py-3">
    <div class="d-flex gap-2 flex-wrap">
      ...
      <a href="?status=<?= $val ?>"
        ...
        <?= $lbl ?>
        <?php if ($val !== 'all'):
          $cnt = $db->prepare("SELECT COUNT(*) FROM documents WHERE status=?");
          $cnt->execute([$val]);
          echo '<span class="badge bg-secondary ms-1">'.$cnt->fetchColumn().'</span>';
        endif; ?>
      </a>
      <?php endforeach; ?>
    </div>
