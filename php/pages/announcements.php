...
requireLogin();
define('PAGE_TITLE', 'Announcements');
$db = getDB();

if ($_SERVER['REQUEST_METHOD'] === 'POST' && hasRole('admin','secretary')) {
    if ($_POST['action'] === 'add') {
        $db->prepare("INSERT INTO announcements (type,title,content,date,status) VALUES (?,?,?,?,?)")
           ->execute([$_POST['type'],$_POST['title'],$_POST['content'],$_POST['date'],'active']);
    ...
    header('Location: ' . BASE_URL . 'announcements'); exit;
}

$filter = $_GET['type'] ?? 'all';
$where  = $filter !== 'all' ? "WHERE type=?" : "";
$params = $filter !== 'all' ? [$filter] : [];
$stmt   = $db->prepare("SELECT * FROM announcements $where ORDER BY date DESC");
$stmt->execute($params);
