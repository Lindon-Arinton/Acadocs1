...
requireLogin();
if (!hasRole('admin')) { header('Location: ' . BASE_URL . 'dashboard'); exit; }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add') {
        ...
        $db->prepare("INSERT INTO users (name,email,password,role) VALUES (?,?,?,?)")
           ->execute([$_POST['name'],$_POST['email'],$hash,$_POST['role']]);
        $_SESSION['flash'] = ['type'=>'success','msg'=>'User created successfully.'];
    } elseif ($action === 'delete' && (int)$_POST['id'] !== (int)currentUser()['id']) {
...
$search = trim($_GET['q'] ?? '');
$stmt = $db->prepare("SELECT id,name,email,role,created_at FROM users WHERE name LIKE ? OR email LIKE ? ORDER BY role,name");
        ...
        <tbody>
          ...
          <tr>
            ...
            <td class="text-center">
              ...
              <?php if (!$isMe): ?>
              <form method="POST" class="d-inline" onsubmit="return confirm('Delete <?= e(addslashes($u['name'])) ?>?')">
