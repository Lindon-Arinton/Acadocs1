...
requireLogin();
define('PAGE_TITLE', 'Property Management');
$db = getDB();

if ($_SERVER['REQUEST_METHOD'] === 'POST' && hasRole('admin')) {
    $action = $_POST['action'] ?? '';
    if ($action === 'add') {
        $db->prepare("INSERT INTO room_properties (room_number,building_name,item_name,quantity,condition_status,last_inspection,remarks) VALUES (?,?,?,?,?,?,?)")
           ->execute([$_POST['room_number'],$_POST['building_name'],$_POST['item_name'],
                      (int)$_POST['quantity'],$_POST['condition_status'],$_POST['last_inspection']??date('Y-m-d'),$_POST['remarks']??'']);
        $_SESSION['flash'] = ['type'=>'success','msg'=>'Item added successfully.'];
    } elseif ($action === 'delete') {
        $db->prepare("DELETE FROM room_properties WHERE id=?")->execute([(int)$_POST['id']]);
        $_SESSION['flash'] = ['type'=>'success','msg'=>'Item removed.'];
    }
    header('Location: ' . BASE_URL . 'property-management'); exit;
}
...
$where = []; $params = [];
if ($building  !== 'all') { $where[] = 'building_name=?';    $params[] = $building; }
if ($condition !== 'all') { $where[] = 'condition_status=?'; $params[] = $condition; }
...
$buildings  = $db->query("SELECT DISTINCT building_name FROM room_properties ORDER BY building_name")->fetchAll(PDO::FETCH_COLUMN);
